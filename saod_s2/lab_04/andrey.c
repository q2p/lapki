...
char BTreeNode_search(BTreeNode* node, int k) {
	int i = 0;
...
char BTree_contains(BTree* tree, int k) {
...
RBNode* rbtree_new_node(int val) {
...
char rbtree_is_on_left(RBNode* node) {
...
void rbtree_delete_node(RBTree* tree, RBNode *node) {
  ...
  char uvBlack = (u == NULL || !u->is_red) && !node->is_red;
...
RBNode* rbtree_search(RBNode* node, int key) {
  while (1) {
		if (key == node->val) {
  		return node;
    }
...
void insertKey(BTree* btree, RBTree* rbtree, int key) {
	printf("Inserting %d\n", key);
	BTree_insert(btree, key);
	rbtree_insert(rbtree, key);
}

void removeKey(BTree* btree, RBTree* rbtree, int key) {
	printf("Removing %d\n", key);
	BTree_remove(btree, key);
	rbtree_remove(rbtree, key);
}
